...
from src.top_collect_with_ya_metrics.bukvarix_collector import run_collect_keywords_top
from src.top_collect_with_ya_metrics.run_top import run_collect_top_links
...
from core.logger import get_logger, log_exception, app_logger
from core.config import DEFAULT_THREADS_COUNT, MAX_LINKS
...
class Application:
    ...
    def _show_menu(self) -> None:
        ...
        print("1. Режим сбора ссылок по сервисам яндекса")
        print("2. Режим сбора ссылок из топа Bukvarix")
        print("2. Выйти")

    def _run_ya_service_mode(self) -> None:
        ...
        try:
            ...
            run_create_result("regular")  # Создаем финальный результат в обычном режиме
        except Exception as e:
            log_exception(self.logger, "Ошибка в режиме одного файла", e)
            print(f"Произошла ошибка: {str(e)}")

    def _run_top_mode(self) -> None:
        """Запускает режим одного файла."""
        try:
            self.num_threads = int(
                input(f"Введите количество потоков (по умолчанию {DEFAULT_THREADS_COUNT}): ") or DEFAULT_THREADS_COUNT)
            self.max_links_per_query = int(input(
                f"Введите количество ссылок для каждого запроса (макс. {MAX_LINKS}, по умолчанию {MAX_LINKS}): ") or MAX_LINKS)
            ...
            # paths.cleanup()  # Очищаем директории
            # run_collect_domains()  # Собираем домены
            #run_collect_keywords_top()  # Собираем ключевые слова
            run_collect_top_links(self.num_threads, self.max_links_per_query)  # Собираем ссылки
            run_create_result("top")  # Создаем финальный результат в ТОП-режиме
        except Exception as e:
            log_exception(self.logger, "Ошибка в режиме одного файла", e)
            print(f"Произошла ошибка: {str(e)}")
